...
| `AbilityOvershootCastRange` | Integer | 施法过冲距离（允许超出施法距离的最大范围） |
...
通过设置 `BaseClass` 为原生技能名，可继承其完整逻辑，仅覆写数值参数。
...
"creature_black_hole"
{
    "BaseClass"            "enigma_black_hole"    // 继承原生技能
    "AbilityType"          "DOTA_ABILITY_TYPE_ULTIMATE"
    "AbilityCastRange"     "275"                   // 覆写参数
...
- 继承的技能保留原生逻辑，仅覆写 KV 中定义的参数
- 如需修改行为逻辑，应改为使用 `ability_lua` 或 `ability_datadriven` 复刻
- 继承时 `AbilityTextureName` 会自动使用原生图标，可手动覆写

## 覆写与禁用

### 覆写原生技能

在 `npc_abilities_override.txt` 或 `npc_abilities_custom.txt` 中，使用与原生技能相同的 key 即可覆写：

```kv
"spirit_breaker_charge_of_darkness"
{
    "AbilityCooldown"    "7"        // 覆写冷却
    "AbilityManaCost"    "0"        // 覆写蓝耗
}
```

### 禁用原生物品

通过设置 `ItemPurchasable` 和 `ItemDroppable` 为 `0` 禁用物品：
...
| 物品 | 禁用原因 |
|------|----------|
| `item_courier` / `item_flying_courier` | 自定义模式通常不需要信使 |
| `item_tpscroll` | 防止传送逃脱 |
| `item_bottle` | 防止控符 |
| `item_smoke_of_deceit` | 防止隐身集结 |
| `item_hand_of_midas` | 防止加速发育 |
| `item_tome_of_knowledge` | 防止购买经验 |
